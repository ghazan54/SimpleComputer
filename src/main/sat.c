...
int
sat (const char *filepath, const char *result)
{
  FILE *file = fopen (filepath, "r");
  if (!file)
    {
      fprintf (stderr,
               "sat:\e[31m fatal error:\e[39m %s: There is no such file or "
               "directory\n",
               filepath);
      return ERROR_CODE;
    }
  ...
  while (!feof (file))
    {
      ...
      if ((!i && strcmp (tok, "00")) || i >= MEMORY_SIZE)
        {
          fprintf (stderr,
                   "sat:%d\e[31m fatal error:\e[39m %s: Out of range\n",
                   count_strs, tok);
          return ERROR_CODE;
        }
      ...
      if (command == -1)
        {
          fprintf (stderr,
                   "sat:%d\e[31m fatal error:\e[39m %s: Unknown command\n",
                   count_strs, tok);
          return ERROR_CODE;
        }
      ...
      if (!operand && (strcmp (tok_tmp, "00") && strcmp (tok_tmp, "0000")))
        {
          fprintf (stderr,
                   "sat:%d\e[31m fatal error:\e[39m %s: Invalid operand\n",
                   count_strs, tok);
          return ERROR_CODE;
        }
      ...
      if (tok && *tok != ';')
        {
          fprintf (stderr,
                   "sat:%d\e[31m syntax error:\e[39m %s: The ';' character "
                   "was expected\n",
                   count_strs, tok);
          return ERROR_CODE;
        }
          ...
          if (sc_commandEncode (command, operand, &memory[i]))
            {
              fprintf (
                  stderr,
                  "sat:%d\e[31m fatal error:\e[39m sc_commandEncode(): -1\n",
                  count_strs);
              return ERROR_CODE;
            }
        }
  ...
  if (!data)
    {
      fprintf (stderr,
               "sat:\e[31m fatal error:\e[39m %s: Failed to save result\n",
               filepath);
      return ERROR_CODE;
    }
