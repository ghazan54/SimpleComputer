...
int bc_printA(const char* str) { return str && !sc_print("\e(0%s\e(B", str) ? SUCCES_CODE : ERROR_CODE; }

int bc_box(int x1, int y1, const int x2, const int y2) {
    if (x1 > x2 || y1 > y2 || x1 < 0 || x2 < 0 || y1 < 0 || y2 < 0) return ERROR_CODE;
    bool first = true;
    for (; x1 <= x2; ++x1, first = false) {
        if (mt_gotoXY(x1, y1)) return ERROR_CODE;
        if (first) {
            for (int _y1 = y1; _y1 <= y2; ++_y1) {
                if (_y1 == y1) {
                    if (bc_printA(ACS_ULCORNER)) return ERROR_CODE;
                } else if (_y1 == y2) {
                    if (bc_printA(ACS_URCORNER)) return ERROR_CODE;
                } else {
                    if (bc_printA(ACS_HLINE)) return ERROR_CODE;
                }
            }
        } else if (x1 == x2) {
            for (int _y1 = y1; _y1 <= y2; ++_y1) {
                if (_y1 == y1) {
                    if (bc_printA(ACS_LLCORNER)) return ERROR_CODE;
                } else if (_y1 == y2) {
                    if (bc_printA(ACS_LRCORNER)) return ERROR_CODE;
                } else {
                    if (bc_printA(ACS_HLINE)) return ERROR_CODE;
                }
            }
...
int bc_bigcharwrite(int fd, int* big, int count) {
    return !big || write(fd, big, count * sizeof(int) * 2) == -1 ? ERROR_CODE : SUCCES_CODE;
}

int bc_bigcharread(int fd, int* big, int need_count, int* count) {
    if (!big || !count) return ERROR_CODE;
