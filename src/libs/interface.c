...
int I_scstep(int rignore) {
    ...
    if (startcu == 2) {
        CU();
        if (!halt_ex) return I_runprogram();
    } else if (startcu == 1) {
        CU();
...
int I_printall(void) {
    if (I_printinfo('a', color_default, color_default)) return ERROR_CODE;
    if (I_printinfo('i', color_default, color_default)) return ERROR_CODE;
    if (I_printinfo('o', color_default, color_default)) return ERROR_CODE;
    if (I_printinfo('f', color_default, color_default)) return ERROR_CODE;
    if (I_printcustomfields()) return ERROR_CODE;
    if (mt_gotoXY(1, 29)) return ERROR_CODE;
    sc_print(" Memory ");
...
int I_printhex(int ic, enum colors fg, enum colors bg) {
    ...
    if (i >= DEFAULT_MAX_STRS || j >= DEFAULT_MAX_STRS || i < 0 || j < 0) {
        ...
        if (mt_gotoXY(i + base_x, j ? j * 6 + base_y : base_y)) return ERROR_CODE;
        sc_memoryAddressOutput(i, j);
    }
    if (mt_setfgcolor(color_default)) return ERROR_CODE;
    if (mt_setbgcolor(color_default)) return ERROR_CODE;
    return SUCCES_CODE;
}
...
int I_printbig(int ic) {
    ...
    char digit[5];
    int command, operand;
    if (sc_commandDecode(c, &command, &operand)) return ERROR_CODE;
...
int I_printkeys(void) {
    if (mt_gotoXY(13, 48)) return ERROR_CODE;
    sc_print(" Keys ");
    if (mt_gotoXY(14, 48)) return ERROR_CODE;
    sc_print("l - load\n");
    if (mt_gotoXY(15, 48)) return ERROR_CODE;
    sc_print("s - save\n");
    if (mt_gotoXY(16, 48)) return ERROR_CODE;
    sc_print("r - run\n");
    if (mt_gotoXY(17, 48)) return ERROR_CODE;
    sc_print("t - step\n");
    if (mt_gotoXY(18, 48)) return ERROR_CODE;
    sc_print("F5 - accumulator\n");
    if (mt_gotoXY(19, 48)) return ERROR_CODE;
    sc_print("F6 - instructionCounter\n");
    return SUCCES_CODE;
}

int I_printaccumulator() {
    if (mt_gotoXY(1, 67)) return ERROR_CODE;
...
int I_printinstructionCounter() {
    if (mt_gotoXY(4, 64)) return ERROR_CODE;
    sc_print(" instructionCounter ");
    if (mt_gotoXY(I_POS_IC_X, I_POS_IC_Y)) return ERROR_CODE;
...
int I_printflags() {
    if (mt_gotoXY(10, 70)) return ERROR_CODE;
    sc_print(" Flags ");
    if (mt_gotoXY(I_POS_FLAGS_X, I_POS_FLAGS_Y)) return ERROR_CODE;
    int r = 0, c;
...
int I_printcustomfields() {
    ...
    if (mt_gotoXY(23, 3)) return ERROR_CODE;
    sc_print(" Input ");
    if (mt_gotoXY(24, 2)) return ERROR_CODE;
    sc_print(" > ");
    if (mt_gotoXY(23, 49)) return ERROR_CODE;
    sc_print(" Output ");
    return SUCCES_CODE;
}
...
int I_restartsc(void) {
    if (sc_memoryFree() || sc_memoryInit() || sc_regInit()) return ERROR_CODE;
    struct itimerval nval, oval;
    nval.it_interval.tv_sec = 0;
